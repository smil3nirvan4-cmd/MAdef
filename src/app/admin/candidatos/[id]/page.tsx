...
export default function CandidatoDetailPage() {
    ...
    if (!cuidador) return <div className="p-8 text-center text-error-500">Candidato não encontrado</div>;
    ...
    return (
        <div className="p-6 lg:p-8">
            ...
            <div className="grid lg:grid-cols-3 gap-6">
                ...
                <Card>
                    <div className="text-center mb-4">
                        <div className="w-20 h-20 bg-primary-100 rounded-full mx-auto flex items-center justify-center text-3xl mb-3">
                    ...
                    <div className="space-y-3 text-sm">
                        ...
                        <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-muted-foreground" />{new Date(cuidador.createdAt).toLocaleDateString('pt-BR')}</div>
                    </div>
                    <hr className="my-4" />
                ...
                <Card>
                    ...
                    <div className="space-y-3">
                        <Button className="w-full justify-start bg-secondary-500 hover:bg-secondary-600" onClick={() => handleAction('whatsapp')}>
                        ...
                        {cuidador.status === 'REJEITADO' && (
                            <Button className="w-full justify-start bg-accent-500 hover:bg-accent-600" onClick={() => handleAction('reativar')} isLoading={actionLoading === 'reativar'}>
                                <RefreshCw className="w-4 h-4" />Reativar
                            </Button>
                        )}
                        <hr />
                        <Button className="w-full justify-start" variant="outline" onClick={() => handleAction('delete')}>
                            <Trash2 className="w-4 h-4 text-error-500" /><span className="text-error-500">Excluir</span>
                        </Button>
                    </div>
                </Card>
                ...
                <Card>
                    ...
                    {cuidador.alocacoes?.length === 0 ? <p className="text-muted-foreground text-sm">Nenhuma alocação</p> :
                        <div className="space-y-2 max-h-60 overflow-y-auto">
                            {cuidador.alocacoes?.map(a => (
                                <div key={a.id} className="p-2 bg-background rounded text-sm flex justify-between items-center">
                                    <div>
                                        <p className="font-medium">{a.paciente?.nome || 'Sem paciente'}</p>
                                        <p className="text-xs text-muted-foreground">{a.turno} - {new Date(a.dataInicio).toLocaleDateString('pt-BR')}</p>
                                    </div>
            ...
            <Card className="mt-6">
                ...
                <div className="max-h-80 overflow-y-auto space-y-2 bg-background p-4 rounded-lg">
