            ...
            {showAdd && (
                <Card>
                    <div className="grid md:grid-cols-4 gap-4">
                        ...
                        <div><label className="text-sm text-gray-600">Cor</label><input type="color" value={newLabel.color} onChange={(e) => setNewLabel({ ...newLabel, color: e.target.value })} className="w-full h-10 rounded border mt-1" /></div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {labels.map((l) => (
                    <Card key={l.id} className="flex items-center gap-3">
                        ...
                        <div className="flex-1"><p className="font-medium">{l.name}</p><p className="text-xs text-gray-500">{l.description}</p></div>
                        <Button size="sm" variant="ghost" onClick={() => handleDelete(l.id)}><X className="w-4 h-4 text-red-500" /></Button>
                    </Card>
                ))}
            </div>
        </div>
        ...
        <div className="space-y-6">
            ...
            <Card noPadding>
                <table className="w-full text-sm">
                    <thead className="bg-gray-50"><tr><th className="px-4 py-3 text-left">Telefone</th><th className="px-4 py-3 text-left">Motivo</th><th className="px-4 py-3 text-left">Data</th><th className="px-4 py-3 text-right">Ação</th></tr></thead>
                    <tbody className="divide-y">
                        {list.map((b) => (<tr key={b.phone}><td className="px-4 py-3 font-mono">{b.phone}</td><td className="px-4 py-3 text-gray-600">{b.reason || '-'}</td><td className="px-4 py-3 text-gray-500">{new Date(b.createdAt).toLocaleDateString('pt-BR')}</td><td className="px-4 py-3 text-right"><Button size="sm" variant="ghost" onClick={() => handleRemove(b.phone)}><Trash2 className="w-4 h-4 text-green-600" /></Button></td></tr>))}
                    </tbody>
                </table>
                {list.length === 0 && <p className="p-8 text-center text-gray-500">Nenhum número bloqueado</p>}
            </Card>
        </div>
    );
}
            ...
            {showAdd && (
                <Card>
                    <div className="grid md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div><label className="text-sm text-gray-600">Tipo de Trigger</label><select value={newRule.triggerType} onChange={(e) => setNewRule({ ...newRule, triggerType: e.target.value })} className="w-full border rounded-lg px-3 py-2 mt-1">{triggerTypes.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
                    </div>
                    ...
                    <div className="mb-4"><label className="text-sm text-gray-600">Resposta</label><textarea className="w-full border rounded-lg p-3 h-24 mt-1" value={newRule.response} onChange={(e) => setNewRule({ ...newRule, response: e.target.value })} /></div>
            ...
            <div className="space-y-3">
                {rules.map((r) => (
                    <Card key={r.id} className="flex items-start gap-4">
                        <button onClick={() => handleToggle(r.id, !r.active)} className={`w-12 h-6 rounded-full ${r.active ? 'bg-green-500' : 'bg-gray-300'} flex-shrink-0`}><div className={`w-5 h-5 bg-white rounded-full shadow transform ${r.active ? 'translate-x-6' : 'translate-x-0.5'}`} /></button>
                        <div className="flex-1 min-w-0">
                            <p className="font-semibold">{r.name}</p>
                            <p className="text-xs text-gray-500"><Badge variant="info">{r.triggerType}</Badge> {r.trigger}</p>
                            <p className="text-sm text-gray-600 mt-1 whitespace-pre-wrap">{r.response}</p>
                        </div>
                        <Button size="sm" variant="ghost" onClick={() => handleDelete(r.id)}><Trash2 className="w-4 h-4 text-red-500" /></Button>
                    </Card>
                ))}
            </div>
        </div>
    ...
    return (
        <div className="space-y-6">
            <div className="grid grid-cols-3 gap-4">
                <Card className="!p-4 cursor-pointer" onClick={() => setFilter('pending')}><p className="text-2xl font-bold text-yellow-600">{stats?.pending || 0}</p><p className="text-xs text-gray-500">Pendentes</p></Card>
                <Card className="!p-4 cursor-pointer" onClick={() => setFilter('sent')}><p className="text-2xl font-bold text-green-600">{stats?.sent || 0}</p><p className="text-xs text-gray-500">Enviados</p></Card>
                <Card className="!p-4 cursor-pointer" onClick={() => setFilter('failed')}><p className="text-2xl font-bold text-red-600">{stats?.failed || 0}</p><p className="text-xs text-gray-500">Falhas</p></Card>
            </div>
            ...
            <Card noPadding>
                <table className="w-full text-sm">
                    <thead className="bg-gray-50"><tr><th className="px-4 py-3 w-10"><input type="checkbox" onChange={(e) => setSelected(e.target.checked ? queue.map(q => q.id) : [])} /></th><th className="px-4 py-3 text-left">Telefone</th><th className="px-4 py-3 text-left">Mensagem</th><th className="px-4 py-3 text-left">Data</th></tr></thead>
                    <tbody className="divide-y">
                        {queue.map((q) => (<tr key={q.id} className="hover:bg-gray-50"><td className="px-4 py-3"><input type="checkbox" checked={selected.includes(q.id)} onChange={(e) => setSelected(e.target.checked ? [...selected, q.id] : selected.filter(s => s !== q.id))} /></td><td className="px-4 py-3 font-mono text-xs">{q.telefone?.replace('@s.whatsapp.net', '')}</td><td className="px-4 py-3 text-gray-600 truncate max-w-xs">{q.conteudo}</td><td className="px-4 py-3 text-gray-500">{new Date(q.timestamp).toLocaleString('pt-BR')}</td></tr>))}
                    </tbody>
                </table>
                {queue.length === 0 && <p className="p-8 text-center text-gray-500">Nenhuma mensagem na fila</p>}
            </Card>
        </div>
    );
}
            ...
            <div className="space-y-3">
                {webhooks.map((wh) => (
                    <Card key={wh.id} className="flex items-center gap-4">
                        <button onClick={() => handleToggle(wh.id, !wh.active)} className={`w-12 h-6 rounded-full ${wh.active ? 'bg-green-500' : 'bg-gray-300'}`}><div className={`w-5 h-5 bg-white rounded-full shadow transform ${wh.active ? 'translate-x-6' : 'translate-x-0.5'}`} /></button>
                        <div className="flex-1"><p className="font-semibold">{wh.name}</p><p className="text-xs text-gray-500 font-mono">{wh.url}</p><div className="flex gap-1 mt-1">{wh.events?.map((e: string) => <Badge key={e} variant="info">{e}</Badge>)}</div></div>
                        <Button size="sm" variant="ghost" onClick={() => handleDelete(wh.id)}><Trash2 className="w-4 h-4 text-red-500" /></Button>
                    </Card>
                ))}
            </div>
        </div>
    ...
    return (
        <div className="grid lg:grid-cols-2 gap-6">
            <Card>
                ...
                <p className="text-sm text-gray-600 mb-4">Exporta todas as configurações do WhatsApp: automação, templates, respostas rápidas, auto-respostas, webhooks, etiquetas e lista negra.</p>
            ...
            <Card>
                ...
                <p className="text-sm text-gray-600 mb-4">Restaura configurações de um arquivo de backup exportado anteriormente.</p>
                ...
                {result && <div className={`mt-4 p-3 rounded-lg ${result.success ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>{result.success ? `✅ ${result.imported} configurações importadas` : `❌ ${result.error}`}</div>}
            </Card>
        </div>
    );
}
